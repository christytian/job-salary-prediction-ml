"""
Complete Feature Evaluation with Random Forest (NLP + Optimized Categorical Encoding)
=====================================================================================

This script evaluates the quality and effectiveness of the complete feature set using Random Forest:
...
2. Predictive power: Random Forest model performance with all features
3. Feature importance: Top contributing features from Random Forest
"""
...
def evaluate_predictive_power(data: List[Dict], fieldnames: List[str]) -> Dict:
    """
    Evaluate predictive power of hybrid features using Random Forest.
    ...
    print("\n" + "=" * 80)
    print("PREDICTIVE POWER EVALUATION (Random Forest)")
    print("=" * 80)
    ...
    # Random Forest parameters - can be tuned for better performance
    ...
    # Feature importance (Random Forest provides feature_importances_)
    feature_importance = model.feature_importances_
    top_features_idx = np.argsort(feature_importance)[-20:][::-1]  # Top 20 features
    
    print("\n🔝 Top 20 Most Important Features (by Random Forest importance):")
...
def evaluate_hybrid_features(input_file: str):
    """
    Comprehensive evaluation of complete feature set (NLP + Optimized Categorical) using Random Forest.
    
    Args:
        input_file: Path to CSV file with all features (NLP + optimized categorical encoding)
    """
    print("=" * 80)
    print("COMPLETE FEATURE EVALUATION WITH RANDOM FOREST")
    ...
    if predictive_results:
        print("\n✅ Predictive Power (Random Forest):")
        ...
        print("   All features (Hybrid NLP + Optimized Categorical) with Random Forest:")
        print(f"   Test R²: {predictive_results['test_r2']:.4f}")
        print(f"   Test RMSE: ${predictive_results['test_rmse']:,.0f}")
        print(f"   Test MAE: ${predictive_results['test_mae']:,.0f}")
