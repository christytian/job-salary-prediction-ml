"""
Complete Feature Evaluation (NLP + Optimized Categorical Encoding)
===================================================================

This script evaluates the quality and effectiveness of the complete feature set:
...
2. Predictive power: Linear Regression model performance with all features
...
def evaluate_predictive_power(data: List[Dict], fieldnames: List[str]) -> Dict:
    """
    Evaluate predictive power of hybrid features using a simple linear model.
    ...
    # Train model with regularization to handle high dimensionality
    print("\n🤖 Training Linear Regression model...")
    print("   Using Ridge Regression (L2 regularization) for numerical stability...")
    
    # Try Ridge first (more stable for high-dimensional data)
    model = Ridge(alpha=1.0, max_iter=1000)
    model.fit(X_train_scaled, y_train)
    
    # If Ridge fails or performs poorly, fall back to LinearRegression
    try:
        y_test_pred_ridge = model.predict(X_test_scaled)
        test_r2_ridge = r2_score(y_test, y_test_pred_ridge)
        if test_r2_ridge < -100:  # Unstable, try LinearRegression
            print("   Ridge unstable, trying LinearRegression...")
            model = LinearRegression()
            model.fit(X_train_scaled, y_train)
    except Exception:
        print("   Ridge failed, using LinearRegression...")
        model = LinearRegression()
        model.fit(X_train_scaled, y_train)
    ...
    top_features_idx = np.argsort(coef_abs)[-10:][::-1]
    
    print(f"\n🔝 Top 10 Most Important Features (by coefficient magnitude):")
    for idx in top_features_idx:
        print(f"   {feature_cols[idx]}: {model.coef_[idx]:.6f}")
...
def evaluate_hybrid_features(input_file: str):
    """
    Comprehensive evaluation of complete feature set (NLP + Optimized Categorical).
    
    Args:
        input_file: Path to CSV file with all features (NLP + optimized categorical encoding)
    """
    print("=" * 80)
    print("COMPLETE FEATURE EVALUATION")
    ...
    if predictive_results:
        ...
        print(f"   All features (NLP + Optimized Categorical):")
        print(f"   Test R²: {predictive_results['test_r2']:.4f}")
        print(f"   Test RMSE: ${predictive_results['test_rmse']:,.0f}")
        print(f"   Test MAE: ${predictive_results['test_mae']:,.0f}")
